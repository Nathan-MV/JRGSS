...
public class JRGSSGame implements JRGSSApplicationListener {
    ...
    public static void jrgssMain(JRGSSMain rubyBlock) {
        JRGSSLogger.println(PEDANTIC,"Got the Ruby Runtime");
        mainBlock = rubyBlock;
        JRGSSGame.runWithGLContext(() -> { mainBlock.main(); });
    }
    ...
    @Override
    public void pause() {
        JRGSSLogger.println(PEDANTIC,"Pause");
    }

    @Override
    public void resume() {
        JRGSSLogger.println(PEDANTIC,"Resume");
    }
    ...
    public void update() {
        JRGSSLogger.println(PEDANTIC,"Update");
